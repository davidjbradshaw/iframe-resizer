...
##Version History
* v1.4.1 Setting `interval` to a negative number now forces the interval test to run instead of [MutationObserver](https://developer.mozilla.org/en/docs/Web/API/MutationObserver). This is useful in cases where you have a lot of rapid changes to the DOM as the browser will throttle calls to MutationObserver.
