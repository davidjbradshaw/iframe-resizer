...
This test captures and validates console log output from iframe-resizer during a comprehensive user interaction workflow. It runs against both **dev** (js/) and **prod** (js-dist/) builds to ensure consistent logging behavior.
...
The test uses Playwright's route interception to load different builds:

- **Dev Build** (`console-dev` project): Uses `js/` directory
  - Built with `DEBUG=1`
  - Includes full debug logging and test code
  - Generated by `npm run build:dev`

- **Prod Build** (`console-prod` project): Uses `js-dist/` directory
  - Production optimized
  - Debug code stripped out
  - Generated by full `npm run build:prod`
...
If wrong build is being used:
- Check Playwright route interception is working
- Verify js/ and js-dist/ directories exist
- Check HTTP server is serving all directories correctly
- Use DevTools Network tab to see which scripts load
