 ...
 * and captures all console log output. It runs against both dev (js/)
 * and prod (dist/) builds to ensure consistent behavior.
 *
...
for (const buildType of ['dev', 'prod']) {
  test.describe(`Console log snapshot - ${buildType} build`, () => {
    let consoleLogs = []
    ...
    test.beforeEach(async ({ page }) => {
      ...
      // Intercept script requests and route to appropriate build
      // Dev build uses js/ (DEBUG=1, full logging)
      // Prod build uses js-dist/ (production, optimized)
      await page.route('**/js/iframe-resizer.*.js', async route => {
        const url = route.request().url()

        if (buildType === 'prod') {
          // Replace js/ with js-dist/ for prod builds
          const newUrl = url.replace('/js/', '/js-dist/')
          await route.continue({ url: newUrl })
        } else {
          // Use js/ for dev builds
      ...
      // Small delay to ensure initial logs are captured
      await page.waitForTimeout(500)
    })

    test('should produce consistent console output through user interactions', async ({ page }) => {
      ...
      await test.step('Nested navigation (1)', async () => {
        ...
        await testIframe.locator('a#nested, a[href*="frame.nested.html"]').first().click()
      ...
      await test.step('Nested navigation (2) - within nested iframe', async () => {
        const testIframe = page.frameLocator('#testFrame')
        const nestedIframe = testIframe.frameLocator('#nestedIFrame, iframe[id^="nestedIFrame"]')
        const nestedLink = nestedIframe.locator('a#nested, a[href*="frame.nested.html"]').first()
      ...
      await test.step('Nested in frame 1', async () => {
        ...
        await frame1.locator('a#nested, a[href*="frame.nested.html"]').first().click()
        await page.waitForTimeout(500)
      })
      ...
      const logSnapshot = consoleLogs.map(log =>
        `[${log.type.toUpperCase()}] ${log.text}`
      ).join('\n')

      // Save snapshot
      expect(logSnapshot).toMatchSnapshot(`console-logs-${buildType}.txt`)
