...
vi.mock('../console', () => ({ endAutoGroup: vi.fn(), event: vi.fn(), log: vi.fn() }))
...
const preModeCheck = (await import('../checks/mode')).preModeCheck
...
describe('core/setup/index', () => {
  ...
  test('wires setup sequence when id is unique', async () => {
    ...
    expect(log).toHaveBeenCalledWith('if1', expect.stringContaining('src:'), expect.any(String))
