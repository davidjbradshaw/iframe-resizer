...
vi.mock('../observers/mutation', () => ({ default: vi.fn(() => ({ disconnect: vi.fn() })) }))
vi.mock('../observers/perf', () => ({ default: vi.fn(() => ({ disconnect: vi.fn() })) }))
vi.mock('../observers/visibility', () => ({ default: vi.fn(() => ({ disconnect: vi.fn() })) }))
vi.mock('../size/all', () => ({ getAllElements: vi.fn(() => [document.documentElement]) }))
vi.mock('./mutation', () => ({ default: vi.fn() }))
vi.mock('./overflow', () => ({ default: vi.fn(() => ({ disconnect: vi.fn() })) }))
...
describe('child/observed/index', () => {
  beforeEach(() => { tearDownList.length = 0 })
