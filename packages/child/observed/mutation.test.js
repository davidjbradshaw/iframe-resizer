...
vi.mock('../console', () => ({ endAutoGroup: vi.fn(), event: vi.fn(), info: vi.fn() }))
vi.mock('../send/size', () => ({ default: vi.fn() }))
vi.mock('../size/all', () => ({ getAllElements: vi.fn(() => []) }))
vi.mock('../values/state', () => ({ default: { applySelectors: vi.fn() } }))
vi.mock('./observers', () => ({ default: { overflow: { attachObservers: vi.fn(), detachObservers: vi.fn() }, resize: { attachObserverToNonStaticElements: vi.fn(), detachObservers: vi.fn() } } }))

const { event: consoleEvent } = await import('../console')
...
describe('child/observed/mutation', () => {
  test('handles content mutation and sends size', () => {
    ...
    expect(observers.resize.attachObserverToNonStaticElements).toHaveBeenCalled()
