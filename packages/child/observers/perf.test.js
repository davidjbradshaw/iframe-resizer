...
vi.mock('../console', () => ({ advise: vi.fn(), event: vi.fn(), info: vi.fn(), log: vi.fn() }))
...
describe('child/observers/perf', () => {
  ...
  test('observes marks and advises when threshold exceeded', () => {
    ...
    const calls = []
    globalThis.PerformanceObserver = function(cb) { this.observe = vi.fn(() => { calls.push(cb) }); this.disconnect = vi.fn() }

    const obs = createPerformanceObserver()
    // simulate mark entries many times
    const entry = { name: '--ifr-end', detail: { Side: 'HEIGHT', hasTags: false, len: 30 } }
