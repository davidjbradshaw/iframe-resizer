...
vi.mock('../console', () => ({
  debug: vi.fn(), endAutoGroup: vi.fn(), errorBoundary: (fn) => fn, event: vi.fn(), info: vi.fn(), log: vi.fn(), purge: vi.fn(),
}))
vi.mock('../size/content', () => ({ default: vi.fn(() => ({ height: 10, width: 20 })) }))
vi.mock('../values/settings', () => ({ default: { autoResize: true } }))
vi.mock('../values/state', () => ({ default: { isHidden: false, timerActive: false, totalTime: 0, sameOrigin: false } }))
...
describe('child/send/size', () => {
  ...
  test('ignores when sendPending true for non-overflow triggers', () => {
    ...
    expect(consoleMod.log).toHaveBeenCalledWith('Resize already pending - Ignored resize request')
  })
