...
vi.mock('../values/settings', () => ({ default: { parentId: 'pid', mode: 0, targetOrigin: 'https://p' } }))
vi.mock('../values/state', () => ({ default: { sameOrigin: false, target: { postMessage: vi.fn() }, timerActive: false, totalTime: 0 } }))
...
const { displayTimeTaken, setTargetOrigin, dispatchToParent } = await import('./dispatch')
...
describe('child/send/dispatch', () => {
  ...
  test('dispatchToParent sameOrigin path and mode=1 swallows error via advise', () => {
    ...
    settings.mode = 1
    globalThis.parent = { iframeParentListener: () => { throw new Error('x') } }
