/* eslint-disable @typescript-eslint/no-explicit-any, no-bitwise, no-cond-assign, chai-friendly/no-unused-expressions, pii/no-phone-number */
const l = (l: string): string => {
  ...
  for (let z: any, t = 0; t < l.length; t++)
    ((z = l.codePointAt(t)),
      (p = Math.imul(p ^ z, 2_246_822_519)),
      (y = Math.imul(y ^ z, 3_266_489_917)))
  return (
    (p ^= Math.imul(p ^ (y >>> 15), 1_935_289_751)),
    (y ^= Math.imul(y ^ (p >>> 15), 3_405_138_345)),
    (p ^= y >>> 16),
    (y ^= p >>> 16),
    (2_097_152 * (y >>> 0) + (p >>> 11)).toString(36)
  )
}
const p = (l: string): string =>
  l.replace(/[A-Za-z]/g, (l: any) =>
    String.fromCodePoint(
      (l <= 'Z' ? 90 : 122) >= (l = l.codePointAt(0) + 19) ? l : l - 26,
    ),
  )
const x = ['spjluzl', 'rlf', 'clyzpvu', 'rlf2']
const y = [
...
const z = ['NWSc3', 'zvsv', 'wyv', 'ibzpulzz', 'vlt']
...
/* eslint-enable @typescript-eslint/no-explicit-any, no-bitwise, no-cond-assign, chai-friendly/no-unused-expressions, pii/no-phone-number */
